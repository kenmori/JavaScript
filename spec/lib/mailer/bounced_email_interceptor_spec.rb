RSpec.describe Mailer::BouncedEmailInterceptor do
  describe "#delivering_email" do
    ...
    context 'bounce_emails is empty' do
      it_behaves_like "message.to does not change"
    end

    context 'bounce_emails is not empty' do
      let!(:bounced_emails) { BounceEmailFactory.new.create }

      context 'message.to does not contains bounced email address' do
        it_behaves_like "message.to does not change"
      end

      context 'message.to contains bounced email address' do
