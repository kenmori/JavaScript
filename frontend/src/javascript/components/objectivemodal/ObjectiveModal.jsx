...
class ObjectiveModal extends PureComponent {
  ...
  constructor() {
    ...
    this.panes = [
      ...
      {
        menuItem: "孤立 OKR 紐付け",
        render: () => this.getObjectiveFormHtml({ isLink: true }),
      },
      {
        menuItem: "前期 OKR コピー",
        render: () => this.getObjectiveFormHtml({ isCopy: true }),
      },
    ];
  }
  ...
  componentWillReceiveProps(nextProps) {
    if (!this.props.isOpen && nextProps.isOpen) {
      ...
      this.props.initialize({
        name: "",
        parentKeyResultId: nextProps.parentKeyResult
          ? nextProps.parentKeyResult.get("id")
          : -1,
        objectiveId: null,
      });
    }
  ...
  save(validData) {
    const parentKeyResultId =
      validData.parentKeyResultId !== -1 ? validData.parentKeyResultId : null;
  ...
  getInitialOwnerId(props = this.props) {
    if (props.parentKeyResult) {
      const isMember = props.parentKeyResult
        .get("members")
        .some(member => member.get("id") === props.loginUserId);
      return isMember
        ? props.loginUserId
        : props.parentKeyResult.getIn(["owner", "id"]);
    }
  ...
  getObjectiveFormHtml = ({ isLink = false, isCopy = false }) => (
    <Tab.Pane>
      <ObjectiveForm
        ...
        isFetchedObjectives={
          isLink
            ? this.props.isFetchedObjectives
            : isCopy
            ? this.props.isFetchedPreviousObjectives
            : undefined
        }
      />
    </Tab.Pane>
  ...
  render() {
    ...
    return (
      <Modal
        ...
        <Modal.Header>Objective を入力する</Modal.Header>
        <Modal.Content>
