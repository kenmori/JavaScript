...
class OrganizationSettingTab extends PureComponent {
  ...
  handleNameCommit = name =>
    this.props.updateOrganization({
      id: this.props.organization.get("id"),
      name,
    });
  ...
  handleExportClick = () => {
    ...
    const okrPeriod = okrPeriods.find(
      okrPeriod => okrPeriod.get("id") === okrPeriodId,
    );
    const okrPeriodName = `${okrPeriod.get("name")} (${okrPeriod.get(
      "startDate",
      ...
      <Tab.Pane className="organization-setting-tab">
        <dl>
          ...
          <dd>
            <OkrSpanSelect
              value={okrSpan}
              onChange={this.handleOkrSpanChange}
            />
          </dd>
          <dt>ロゴ</dt>
          <dd>
          ...
          <dd>
            <input
              ...
              ref="fileInput"
              onChange={this.changeLogoImage}
            />
            <Button content="変更する" positive onClick={this.clickFileInput} />
            {path && (
              <Button content="削除する" negative onClick={this.deleteLogo} />
            )}
          </dd>
        </dl>
