...
class OkrPieChart extends PureComponent {
  getPieChartProperties(objective) {
    ...
    const data = objective.get("keyResults").map(keyResult => {
      ...
      return {
        name:
          (keyResult.get("disabled") ? "[無効] " : "") + keyResult.get("name"),
    ...
    return {
      ...
      startAngle,
      endAngle:
        endAngle - Math.round((objective.get("progressRate") / 100) * 360),
      minAngle,
  ...
  getPieChart = objective => {
    const {
      ...
      minAngle,
      label,
    } = this.getPieChartProperties(objective);
    return (
      <PieChart width={145} height={145}>
        <Pie
          ...
          <Label
            value={label}
            position="center"
            className="okr-pie-chart__progress"
          />
        </Pie>
    ...
    return (
      <div className="okr-pie-chart">
        {objective.get("keyResults").isEmpty() ? (
          <div className="empty okr-pie-chart__progress">
            {objective.get("progressRate")}%
          </div>
        ) : (
