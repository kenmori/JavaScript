...
class UsersTable extends SortableComponent {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    this.setState({
      activePage:
        this.props.keyword !== nextProps.keyword ? 1 : this.state.activePage,
    });
  }
  ...
  render() {
    ...
    const end = Math.min(
      filteredUsers.size,
      activePage * UsersTable.NUMBER_TO_DISPLAY,
    );
      ...
      <div className="users-table">
        <Table singleLine sortable>
          <Table.Header>
            <Table.Row>
              <Table.HeaderCell
                sorted={this.isSorted("index")}
                onClick={this.handleSort("index")}
                textAlign="center"
              />
              <Table.HeaderCell disabled />
              <Table.HeaderCell
                sorted={this.isSorted("lastName")}
              ...
              <Table.HeaderCell
                sorted={this.isSorted("isAdmin")}
                onClick={this.handleSort("isAdmin")}>
                権限
              </Table.HeaderCell>
              <Table.HeaderCell
                sorted={this.isSorted("isOwner")}
                onClick={this.handleSort("isOwner")}>
                代表者
              </Table.HeaderCell>
              <Table.HeaderCell
                sorted={this.isSorted("signInAt")}
                onClick={this.handleSort("signInAt")}>
                最終ログイン
              </Table.HeaderCell>
              <Table.HeaderCell disabled />
            </Table.Row>
          ...
          <Table.Footer>
            <Table.Row>
              <Table.HeaderCell colSpan="3">{`${begin + 1} - ${end} / ${
                filteredUsers.size
              } 人`}</Table.HeaderCell>
