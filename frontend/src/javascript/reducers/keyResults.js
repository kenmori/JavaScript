...
function add(state, keyResultId) {
  return state.update("ids", ids =>
    ids.includes(keyResultId) ? ids : ids.insert(0, keyResultId),
  );
}
...
function addToTask(state, keyResultId, payload, orRemove = false) {
  ...
  return state.update("taskIds", ids =>
    ids.includes(keyResultId) ? ids : ids.insert(0, keyResultId),
  );
}
...
export default handleActions(
  {
    [ActionTypes.FETCH_KEY_RESULTS]: state =>
      state.set("isFetchedKeyResults", false),
    [ActionTypes.FETCHED_KEY_RESULTS]: (state, { payload }) =>
      state.set("ids", payload.get("result")).set("isFetchedKeyResults", true),
    ...
    [ActionTypes.FETCHED_TASK_KEY_RESULTS]: (state, { payload }) =>
      state.set("taskIds", payload.get("result")),
    ...
    [ActionTypes.PROCESSED_KEY_RESULT]: (state, { payload }) =>
      removeFromTask(state, payload.id),
    ...
    [ActionTypes.FETCH_KEY_RESULT_COMMENT_LABELS]: state =>
      state.set("isFetchedKeyResultsCommentLabels", false),
    [ActionTypes.FETCHED_KEY_RESULT_COMMENT_LABELS]: (state, { payload }) =>
      state
