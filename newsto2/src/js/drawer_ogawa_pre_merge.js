// side menu  (c) 2014 TENTOMUSHI

//サイドメニュー初期化
window.addEventListener('load',function() {
    //wrapperのセレクター確認
    if(typeof wrapperContentsid == 'undefined'){
        drawerApp.wrapperContentsid = '#wrapper_landscape';
    } else {
        drawerApp.wrapperContentsid = wrapperContentsid;
    }

    //STG用のリストのURLへの変更
    if (typeof drawerJson != 'undefined') {
        drawerApp.requestUrl = drawerJson;
    }

    drawerApp.wrapperContents = jQuery(drawerApp.wrapperContentsid);
    drawerApp.initialize();
});
...
PortalSide.switchAuid  = function (data) {
    var drawerAuid = '';

    if(PortalAd.utils.isIOS() && PortalAd.utils.isTablet()){

        //ページ独自ID確認
        if(typeof drawerAuid_iphone == 'undefined'){
            drawerAuid = data.iPad;
        } else {
            drawerAuid = drawerAuid_ipad;
        }

    } else if(PortalAd.utils.isAndroid() && PortalAd.utils.isTablet()){

        //ページ独自ID確認
        if(typeof drawerAuid_AndroidTab == 'undefined'){
            drawerAuid = data.AndroidTab;
        } else {
            drawerAuid = drawerAuid_AndroidTab;
        }

    } else if (PortalAd.utils.isIOS()) {

        //ページ独自ID確認
        if(typeof drawerAuid_iphone == 'undefined'){
            drawerAuid = data.iPhone;
        } else {
            drawerAuid = drawerAuid_iphone;
        }

    } else if(PortalAd.utils.isAndroid()){

        //ページ独自ID確認
        if(typeof drawerAuid_Android == 'undefined'){
            drawerAuid = data.AndroidSP
        } else {
            drawerAuid = drawerAuid_Android;
        }
...
PortalSide.loader = function () {
    ...
    function _init() {
        ...
        if (PortalAd.settings.idLoaded === false) {
            setTimeout(function () {
                _init();
            }, 20);
        } else {
            var option = (arguments.length == 1 ? arguments[0] : {});

            if (typeof(PortalSide.adset) !== 'undefined') {
                for (var i = 0; i < PortalSide.adset.ads.length; i++) {
                    ad = new MedibaAdPortalClient(PortalSide.adset.ads[i], PortalAd.idsettings)
                    _run();
...
PortalSide.loader.callback = function (json) {
    if (json !== undefined && json.html !== undefined) {
        var html = decodeURIComponent(json.html);
        elem = document.getElementById(PortalSide.adset.ads[0].addivid);
        elem.innerHTML = html;
    }
}
...
var drawerApp = {
    ...
    //表示内容のJSONファイルへのパス
    requestUrl : '//cdn-img.auone.jp/pass/xhr/sidemenu/sidemenu.json',
    ...
    initialize: function () {
        ...
        drawerApp.deviceCheck();

        // 特定条件下では動作を停止させ使用不可にする
        switch(drawerApp.deviceType) {
            case 'App':
            case 'OutOfDevice':
                return false;
                break;
            case 'Android':
                if(drawerApp.osVersion < 2.2) { return false; }
                break;
        }

        drawerApp.wrapperBuild();
        drawerApp.getlistOfjson();
    },

    //デバイス判定
    deviceCheck: function () {

        if (drawerApp.ua.indexOf('spass-app') > 0) {
            //smartpassアプリ
            drawerApp.deviceType = 'App';
        } else if (drawerApp.ua.indexOf('iPhone') > 0) {
            drawerApp.deviceType = 'iphone';
            drawerApp.ua.match(/iPhone OS (\w+){1,3}/g);
        } else if (drawerApp.ua.indexOf('iPad') > 0) {
            drawerApp.deviceType = 'ipad';
            drawerApp.ua.match(/CPU OS (\w+){1,3}/g);
        } else if (drawerApp.ua.indexOf('Android') > 0) {
            drawerApp.deviceType = 'Android';
            drawerApp.osVersion = drawerApp.ua.substr(drawerApp.ua.indexOf('Android') + 8, 3);
            for(var i=0; i<drawerApp.outOfDevices.length; i++) {
                if(drawerApp.ua.indexOf(drawerApp.outOfDevices[i]) > 0) {
                    drawerApp.deviceType = 'OutOfDevice';
                    break;
                }
            }
        } else {
            drawerApp.deviceType = 'pc';
        }

        if (drawerApp.deviceType == "iphone" || drawerApp.deviceType == "ipad") {
            var iosv = (RegExp.$1.replace(/_/g, '') + '00').slice(0, 3);

            if (iosv >= 700) {
                drawerApp.osVersion = 7;
            } else if (iosv >= 600) {
                drawerApp.osVersion = 6;
            } else if (iosv >= 500) {
                drawerApp.osVersion = 5;
            } else if (iosv >= 400) {
                drawerApp.osVersion = 4;
            }
        }
    ...
    wrapperBuild: function () {
        ...
        var drawerWrapper =
            '<div class="t-drawer__wrapper js-t-drawer__scroll">'+
                '<div class="js-t-drawer__modal">'+
                '<nav>'+
                '<div class="sidemenu-ad-box" id="sidemenu-ad-box">'+
                '</div>'+
                '<ul class="js-t-drawer__contents t-drawer__contents"></ul>'+
                '</nav>'+
                '</div>'+
                '</div>';

        var wrapperAfter = '<div class="js-t-drawer__bot t-drawer__bot"></div>';

        jQuery(drawerApp.wrapperContentsid).after(wrapperAfter).prepend(drawerWrapper);


        drawerApp.wrapperLandscape =  jQuery(drawerApp.wrapperContentsid);
        drawerApp.wrapperMain = jQuery('.js-t-wrapper__main');
        drawerApp.pagesideModal  = jQuery('.t-pageside__modal');
        drawerApp.drawerBot = jQuery('.js-t-drawer__bot');
        drawerApp.drawerContents  =  jQuery('.js-t-drawer__contents');
        drawerApp.pageSide = jQuery(".js-t-drawer__scroll");

    },

    getlistOfjson:function (){

        // キャッシュパラメータ：現在ミリ秒を丸め単位で割って整数化して丸め単位を掛ける
        var range = 1800000;
        var cacheParam = (jQuery.now() / range | 0 ) * range;
        var retryMaxCount = 3;
        var retryCount = 0;
        var url = drawerApp.requestUrl + '?_=' + cacheParam;
        ...
        jQuery.ajax({
            ...
            success: function (data) {
                drawerApp.menuData = data;
                drawerApp.menuBuild();
                drawerApp.drawerBot.show();
                drawerApp.drawerSetup();
            }, error: function () {
                if(retryMaxCount > retryCount){
                    retryCount++;
                    cacheParam++;
                    this.url = drawerApp.requestUrl + '?_=' + cacheParam;
                    jQuery.ajax(this);
                }
            }
        });
...
//メニュー生成
    menuBuild: function () {
        ...
        for (var index in drawerApp.menuData.sidemenu) {
            var linkUrl ,listClass, onclickEvent = '';
            var label = drawerApp.menuData.sidemenu[index].name;
            var line = drawerApp.menuData.sidemenu[index].line;
            var indent = drawerApp.menuData.sidemenu[index].indent;
            var iconClass = drawerApp.menuData.sidemenu[index].icon;

            if (drawerApp.deviceType == 'Android') {
                linkUrl = drawerApp.menuData.sidemenu[index].androidlink;
                onclickEvent = drawerApp.menuData.sidemenu[index].touchevent_andro;
            } else if (drawerApp.deviceType == 'iphone' || drawerApp.deviceType == 'ipad' || drawerApp.deviceType == 'pc') {
                linkUrl = drawerApp.menuData.sidemenu[index].iphonelink;
                onclickEvent = drawerApp.menuData.sidemenu[index].touchevent_ios;
            } else {
                linkUrl = '';
            }

            //ログイン/ログアウトに戻り様のURL追加
            if( iconClass == 'drawer__login' || iconClass == 'drawer__logout' ){
                linkUrl = linkUrl + '?targeturl=' + pageUrl;
            }

            if(iconClass){ listClass = iconClass;}
            if(line){ listClass = listClass + ' ' + line;}
            if(indent){ listClass = listClass + ' ' + indent;}

            if (linkUrl) {
                html_pageside = html_pageside +
                    '<li class="'+listClass +'"><a href="' + linkUrl  + '" onClick="' + onclickEvent + '" >' +
                    '<i></i>' + label + '</a></li>';
            }
        }

        jQuery('.js-t-drawer__contents').html(html_pageside);

    },
    drawerSetup: function () {

        //for ipad resize
        if("ipad" == drawerApp.deviceType){
            setTimeout(function () {
                drawerApp.sizeUpdate();
            }, 500);
        }

        drawerApp.landHeight = drawerApp.wrapperLandscape.css('height');
        drawerApp.pageSide.height(drawerApp.landHeight);
        drawerApp.menuWidth = parseInt(drawerApp.pageSide.css('width'));
        ...
        drawerApp.dist = drawerApp.menuWidth;

        if (drawerApp.ua.indexOf('C01') > 0 ||   ( drawerApp.ua.indexOf('HTL22') > 0 && drawerApp.ua.indexOf('Chrome') > 0) || ( drawerApp.ua.indexOf('HTL21') > 0 && drawerApp.ua.indexOf('Chrome') > 0)){
            drawerApp.drawerBot.bind('touchstart click', function(e) {
                drawerApp.botclick(e);
            });
        } else {
            drawerApp.drawerBot.click(function (e) {
                drawerApp.botclick(e);
            });
        }
    ...
    botclick: function (e) {
        ...
        if (typeof ga !== 'function') {
            window.ga = function() {console.log('ga is not function');};
        }
        if (typeof _gaq !== 'object') {
            window._gaq = {
                push: function () {console.log('ga is not function');}
            };
        }

        //万が一、2度押しが発生した場合、ページのreloadを行う
        if(drawerApp.wrapperLandscape.hasClass("safety") && drawerApp.wrapperLandscape.css("position") ==
            "absolute" && drawerApp.wrapperLandscape.css("-webkit-transform") == "matrix(1, 0, 0, 1, 0, 0)"){
            location.reload(true);
        };

        //フェールセーフ用のclassを付与
        drawerApp.wrapperLandscape.addClass("safety");

        if (drawerApp.wrapperLandscape.css("position") == "absolute" && drawerApp.drawertransition) {

            //catchTrEvent送信
            if(typeof trEventBe == 'function'){
                trEventBe(jQuery(e.currentTarget), 'SmartpassOther', 'SideMenu', 'ボタン閉じる', e.originalEvent);
            }
            ...
            //iscroll初期化
            drawerScroll = new IScroll('.js-t-drawer__scroll', {
                mouseWheel: true,
                scrollbars: false,
                click: true
            });
            ...
            //catchTrEvent送信
            if(typeof trEventBe == 'function'){
                trEventBe(jQuery(e.currentTarget), 'SmartpassOther', 'SideMenu', 'ボタン開く', e.originalEvent);
            }

            drawerApp.sizeUpdate();
            drawerApp.slideinDrawer();

            //2度押し防止のため、チャームにpreventDefaultを指定
            drawerApp.drawerBot.on("touchstart.noScroll", function(a) {
                a.preventDefault();
            });
        }
    ...
    sizeUpdate: function () {
        ...
        drawerApp.windowHeight = jQuery(window).height();
        drawerApp.menuWidth = parseInt(drawerApp.pageSide.css('width'));
        drawerApp.scrollWrapper = jQuery(window).scrollTop();

        //パディング追加
        drawerApp.sidemenuPadding = Math.floor(drawerApp.windowHeight*0.15);
        drawerApp.drawerContents.css("padding-bottom", drawerApp.sidemenuPadding + "px");


        if (drawerApp.wrapperLandscape.css("position") == "absolute") {

            if(drawerApp.deviceType == "iphone" && drawerApp.osVersion == 5){
            ...
            drawerApp.pageSide.css({
                height:  window.innerHeight,
                'margin-top': drawerApp.scrollWrapper
            });
    ...
    closeMenu: function () {
        ...
        //2度押し防止の処理を取り除く
        drawerApp.wrapperLandscape.on("webkitTransitionEnd",function(){
            drawerApp.drawerBot.off('.noScroll');
        });

        if (drawerApp.deviceType == "iphone" || drawerApp.deviceType == "ipad") {

            drawerApp.wrapperLandscape.css({
        ...
        if ( drawerApp.deviceType == "Android" && drawerApp.osVersion < 2.4 ){
            drawerApp.drawerBot.css({
                left: '0px'
            });
        ...
        drawerApp.wrapperLandscape.on('webkitTransitionEnd', function () {
            ...
            if (drawerApp.deviceType == "iphone" || drawerApp.deviceType == "ipad" ||  drawerApp.ua.indexOf('A02') > 0 ||
                drawerApp.ua.indexOf('HTL22') > 0 || drawerApp.ua.indexOf('HTL21') > 0 || drawerApp.ua.indexOf('C01') > 0) {
                jQuery(window).off('.noScroll');
            } else {
                drawerApp.wrapperLandscape.off('.noScroll');
            }
            jQuery('.js-t-drawer__cover').remove();
            drawerApp.pageSide.hide();
        });

        jQuery('body').css({width : 'auto'});
        drawerScroll.destroy();
        drawerScroll = null;
        jQuery(".js-drawer__modal").removeAttr('style');

    },

    slideinDrawer: function () {

        //AD初期化
        PortalSide.switchAuid(drawerApp.menuData.auid);
        PortalSide.loader.init();

        drawerApp.pageSide.css({"display": "block"});
        drawerApp.sizeUpdate();
        drawerApp.pageSide.show();

        //スクロール固定
        drawerScroll.drawertransition = false;
        if (drawerApp.deviceType == "iphone" || drawerApp.deviceType == "ipad") {
            jQuery(window).on('touchmove.noScroll', function (e) {
                e.preventDefault();
            });
        } else if ( drawerApp.ua.indexOf('A02') > 0 || drawerApp.ua.indexOf('HTL22') > 0 || drawerApp.ua.indexOf('HTL21') > 0 || drawerApp.ua.indexOf('C01') > 0) {
            jQuery(window).on('touchstart.noScroll', function (e) {
                e.preventDefault();
            });
        } else {
            drawerApp.wrapperLandscape.on('touchmove.noScroll', function (e) {
                e.preventDefault();
            });
        }

        //drawerScrollがnullだった場合、リロードを行う
        if(drawerScroll != null){
            ...
            if (drawerApp.deviceType == "iphone" || drawerApp.deviceType == "ipad") {
                ...
                drawerApp.wrapperLandscape.css({
                    position: "absolute",
                    webkitTransform: "translate3d(" + drawerApp.dist + "px, 0px, 0px)",
                    "-webkit-transition": "-webkit-transform 300ms linear",
                    width: '100%',
                ...
                drawerApp.wrapperLandscape.css({
                    position: "absolute",
                    webkitTransform: "translateX(" + drawerApp.dist + "px)",
                    "-webkit-transition": "-webkit-transform 300ms linear",
                    width: '100%',
                    "-webkit-tap-highlight-color": "rgba(0,0,0,0)"
                });
                ...
                if (drawerApp.deviceType == "Android" && drawerApp.osVersion <= 2.3) {
                    drawerApp.drawerBot.css({
                        left: drawerApp.dist + 'px'
                    });
                } else {
                    drawerApp.drawerBot.css({
                        webkitTransform: "translateX(" + drawerApp.dist + "px)",
            ...
            drawerApp.wrapperLandscape.on('webkitTransitionEnd', function () {
                drawerApp.wrapperLandscape.off('webkitTransitionEnd');
                drawerScroll.refresh();
                drawerScroll.drawertransition = true;
            });
        ...
        var cover = jQuery('<div class="js-t-drawer__cover t-drawer__cover"></div>');

        //サイドメニュー展開後、2度押し防止の処理を解除
        setTimeout(function () {
            drawerApp.drawerBot.off('.noScroll');
        }, 500);
        drawerApp.wrapperLandscape.on("webkitTransitionEnd",function (){
        ...
        //ボタンからのスクロールを禁止
        jQuery('.js-t-drawer__bot').on('touchmove.noScroll', function (e) {
            e.preventDefault();
        });

        jQuery('.js-t-drawer__bot').bind("click", function(e) {
            e.stopPropagation();
        });

        cover.bind('touchend click', function(e) {

            if(drawerScroll.drawertransition){
                //catchTrEvent送信
                if(typeof trEventBe == 'function'){
                    trEventBe(jQuery(e.currentTarget), 'SmartpassOther', 'SideMenu', 'その他閉じる', e.originalEvent);
                }
                drawerApp.closeMenu();
                return false;
            }
        ...
        drawerApp.wrapperLandscape.on("webkitTransitionEnd",function (){
            drawerApp.wrapperLandscape.append(cover);

        });

        //コンテンツ側のスクロールが発生した場合、サイドメニューを閉じる
