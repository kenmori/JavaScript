...
class OrganizationSettingTab extends Component {
  ...
  deleteLogo = (event) => {
    if (confirm('設定済みのアイコンを削除しますか？')) {
      this.props.deleteLogo({id: this.props.organization.get('id'), removeLogo: true});
    }
  }
  render() {
    ...
    return (
      <Tab.Pane attached={false} className="organization-setting-tab">
        <dl>
          <dt>組織名</dt>
          <dd><EditableText value={organization.get('name')} placeholder='会社名やチーム名など' saveValue={name => this.props.updateOrganization({id: organization.get('id'), name})}/></dd>
          <dt>ロゴ</dt>
          <dd>
            <Logo path={path} />
