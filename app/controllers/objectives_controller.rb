...
class ObjectivesController < ApplicationController
  ...
  def show
    ...
    forbidden && return unless valid_permission?(@objective.owner.organization.id)
  end

  def create
    @user = User.find(params[:objective][:owner_id])
    forbidden && return unless valid_permission?(@user.organization.id)
    forbidden("Key Result 責任者または関係者のみ作成できます") && return unless valid_user_to_create?
  ...
  def update
    @objective = Objective.find(params[:id])
    forbidden && return unless valid_permission?(@objective.owner.organization.id)
    forbidden("Objective 責任者のみ編集できます") && return unless valid_user?(@objective.owner.id)
  ...
  def destroy
    @objective = Objective.find(params[:id])
    forbidden && return unless valid_permission?(@objective.owner.organization.id)
    forbidden("Objective 責任者のみ削除できます") && return unless valid_user?(@objective.owner.id)
