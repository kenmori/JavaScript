...
class KeyResultsController < ApplicationController
  def index
    if params[:user_id].present?
      ...
      forbidden && return unless valid_permission?(@user.organization.id)
  ...
  def show_objective
    ...
    forbidden && return unless valid_permission?(key_result.owner.organization.id)
  ...
  def create
    ...
    forbidden && return unless valid_permission?(@user.organization.id)
    forbidden && return unless valid_permission?(owner.organization.id)
    forbidden("Objective 責任者のみ作成できます") && return unless valid_user?(owner.id)
  ...
  def update
    @key_result = KeyResult.find(params[:id])
    forbidden && return unless valid_permission?(@key_result.owner.organization.id)
    forbidden("Objective 責任者または Key Result 責任者のみ編集できます") && return unless valid_user_to_update?
  ...
  def destroy
    @key_result = KeyResult.find(params[:id])
    forbidden && return unless valid_permission?(@key_result.owner.organization.id)
    forbidden("Objective 責任者のみ削除できます") && return unless valid_user?(@key_result.objective.owner.id)
  ...
  def update_processed
    ...
    forbidden && return unless valid_permission?(@key_result.owner.organization.id)
    forbidden("Key Result 責任者または関係者のみ編集できます") && return unless key_result_member
