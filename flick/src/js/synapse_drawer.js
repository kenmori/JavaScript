...
jQuery.event.add(window, "load", function(){
    //wrapperのセレクター確認
    drawerApp.wrapperContentsid = ".js-t-wrapper";
...
var drawerApp = {
    ...
    initialize: function () {
        ...
        //アプリの場合はサイドメニューを停止
        if (drawerApp.deviceType == 'App') {
            return false;
        }

        if (drawerApp.deviceType == 'Android' && drawerApp.osVersion < 2.2) {
            //android 2.1 以下の場合停止
            return false;
        }
    ...
    botclick: function (e) {
        ...
        //20140821　万が一、2度押しが発生した場合、ページのreloadを行う
        ...
        if (drawerApp.wrapperLandscape.css("position") == "absolute" && drawerApp.drawertransition) {
            ...
            //20140821　2度押し防止のため、チャームにpreventDefaultを指定
            drawerApp.drawerBot.on("touchstart.noScroll", function(a) {
                a.preventDefault();
            });

            // メニューを開いたことをトラッキングする synapse_common.jp
            menu.trackShowEvent();
        }
    ...
    slideinDrawer: function () {
        ...
        //20140821　drawerScrollがnullだった場合、リロードを行う
        ...
        //20140821　サイドメニュー展開後、2度押し防止の処理を解除
        setTimeout(function () {
            drawerApp.drawerBot.off('.noScroll');
        }, 500);
        drawerApp.wrapperLandscape.on("webkitTransitionEnd",function (){
