	...
	$.fn.bxSlider = function (options) {
		...
		var init = function () {
			...
			if(slider.settings.loadAnimation) {
				var lastNum = el.children('div').length - 1;
				slider.active = {
					index: lastNum
				};
			} else {
		...
		var start = function () {
			...
			var imgLength = el.find('img').not('.t-bx__clone img').length;
			if(imgLength <= 1) {
				...
				slider.settings.infiniteLoop = false;
			}
			...
			if(slider.settings.showBothImg) {
				...
				el.children('div').css('text-align', 'center');
				var biginNum = 0;
				if((el.children('div')).hasClass('t-bx__clone')) {
					biginNum = biginNum + 1;
				}
				el.children('div').eq(biginNum).addClass('current');
				//一つの時	
				var ImgMargin = 10;
		...
		var onTouchMove = function (e) {
			...
			if(slider.settings.showBothImg) {
				if(change >= 0){
					el.prevAll('.js-clone-prev').remove();
				}else{
					el.prevAll('.js-clone-next').remove();	
				};
				el.children('div').removeClass('current js-t-nextCurrent js-t-prevCurrent');
			};	
		};
		...
		function resize() {	
			var windowW = $(window).width();
			var currentNum = el.children('div').index(el.find('.current'));
			var centerNum = (windowW * currentNum);
			el.css({
		...
		function appearClone(){
			...
			$('.js-clone-next img,.js-clone-prev img').load = function() {
